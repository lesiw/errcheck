...
func init() {
	...
	if err != nil {
		panic(fmt.Errorf(
			"failed to import test package: %v", err,
		))
	}
...
// TestUnchecked runs a test against the example files
// and ensures all unchecked errors are caught.
func TestUnchecked(t *testing.T) {
	test(t, 0)
}

// TestBlank is like TestUnchecked but also ensures
// assignments to the blank identifier are caught.
func TestBlank(t *testing.T) {
	test(t, CheckBlank)
}

func TestAll(t *testing.T) {
	// TODO: CheckAsserts should work independently
	// of CheckBlank
...
func TestBuildTags(t *testing.T) {
	orig := loadPackages
	t.Cleanup(func() { loadPackages = orig })
	const (
		// uses "custom1" build tag and contains
		// 1 unchecked error
		testBuildCustom1Tag = `
` + `// +build custom1
...
func Print1() {
	...
	fmt.Fprintln(nil)
		...
		// uses "custom2" build tag and contains
		// 1 unchecked error
		testBuildCustom2Tag = `
` + `// +build custom2
...
func Print2() {
	...
	fmt.Fprintln(nil)
		...
		// included so that package is not empty
		// when built without specifying tags
		testDoc = `
...
`
	...
	tmpGopath := t.TempDir()
	testBuildTagsDir := path.Join(
		tmpGopath, "src",
		"github.com/testbuildtags",
	)
	...
	if err := os.WriteFile(
		path.Join(testBuildTagsDir, "go.mod"),
		[]byte("module github.com/testbuildtags"),
		0644,
	); err != nil {
		t.Fatalf(
			"Failed to write testbuildtags go.mod: %v",
			err,
		)
	}
	if err := os.WriteFile(
		path.Join(testBuildTagsDir, "custom1.go"),
		[]byte(testBuildCustom1Tag),
		0644,
	); err != nil {
		t.Fatalf(
			"Failed to write testbuildtags custom1: %v",
			err,
		)
	}
	if err := os.WriteFile(
		path.Join(testBuildTagsDir, "custom2.go"),
		[]byte(testBuildCustom2Tag),
		0644,
	); err != nil {
		t.Fatalf(
			"Failed to write testbuildtags custom2: %v",
			err,
		)
	}
	if err := os.WriteFile(
		path.Join(testBuildTagsDir, "doc.go"),
		[]byte(testDoc),
		0644,
	); err != nil {
		t.Fatalf(
			"Failed to write testbuildtags doc: %v",
			err,
		)
	}
	...
	cases := []struct {
		...
		// with no tags specified, main is ignored and
		// there are no errors
		{
			tags:            nil,
			numExpectedErrs: 0,
		},
		// specifying "custom1" tag includes file with
		// 1 error
		{
			tags:            []string{"custom1"},
			numExpectedErrs: 1,
		},
		// specifying "custom1" and "custom2" tags
		// includes 2 files with 1 error each
		{
			tags: []string{
				"custom1", "custom2",
			},
	...
	for _, test := range cases {
		...
		t.Run(testName, func(t *testing.T) {
			...
			packages, err := checker.LoadPackages(
				"github.com/testbuildtags",
			)
			if err != nil {
				t.Fatal(err)
			...
			*uerr = uerr.Unique()
			...
			if test.numExpectedErrs !=
				len(uerr.UncheckedErrors) {
				t.Errorf(
					"expected: %d errors\n"+
						"actual:   %d errors",
					test.numExpectedErrs,
					len(uerr.UncheckedErrors),
				)
			}
		})
...
`
	...
	tmpGopath := t.TempDir()
	testVendorDir := path.Join(
		tmpGopath, "src", "github.com/testvendor",
	)
	...
	if err := os.WriteFile(
		path.Join(testVendorDir, "go.mod"),
		[]byte(testVendorGoMod),
		0755,
	); err != nil {
		t.Fatalf(
			"Failed to write testvendor go.mod: %v",
			err,
		)
	}
	if err := os.WriteFile(
		path.Join(testVendorDir, "main.go"),
		[]byte(testVendorMain),
		0755,
	); err != nil {
		t.Fatalf(
			"Failed to write testvendor main: %v",
			err,
		)
	}
	if err := os.MkdirAll(
	...
	if err := os.WriteFile(
		path.Join(
			testVendorDir,
			"vendor/github.com/testlog/testlog.go",
		),
		[]byte(testLog),
		0755,
	); err != nil {
		t.Fatalf(
			"Failed to write testlog: %v", err,
		)
	}
	...
	cases := []struct {
		...
		{
			ignore: map[string]*regexp.Regexp{
				"github.com/testlog": regexp.MustCompile(
					"Info",
				),
			},
		},
	...
	for i, test := range cases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			checker.Exclusions.GeneratedFiles =
				test.withoutGeneratedCode
			if test.withModVendor {
				if os.Getenv("GO111MODULE") == "off" {
					t.Skip(
						"-mod=vendor doesn't work " +
...
func test(t *testing.T, f flags) {
	...
	checker.Exclusions.Symbols = append(
		...
		DefaultExcludedSymbols...,
	)
	checker.Exclusions.Symbols = append(
		checker.Exclusions.Symbols,
		fmt.Sprintf(
			"(%s.ErrorMakerInterface).MakeNilError",
			testPackage,
		),
	)
	...
	if len(uerr.UncheckedErrors) != numErrors {
		t.Errorf(
			"got %d errors, want %d",
			len(uerr.UncheckedErrors), numErrors,
		)
	unchecked_loop:
	...
	for i, err := range uerr.UncheckedErrors {
		...
		if err.SelectorName != "" &&
			!strings.Contains(
				err.Line, err.SelectorName,
			) {
			t.Errorf(
				"the line '%s' must contain "+
					"the selector '%s'",
				err.Line, err.SelectorName,
			)
		}
